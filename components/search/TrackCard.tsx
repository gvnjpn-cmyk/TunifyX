...
// ── Context Menu ─────────────────────────────────────────────
function ContextMenu({ track, onClose }: { track: Track; onClose: () => void }) {
  const { addToQueue, addNextInQueue, addToPlaylist, playlists, toggleLike, isLiked } = usePlayerStore()
  const liked = isLiked(track.videoId)

  function act(fn: () => void, msg: string) {
    fn(); toast(msg); onClose()
  }

  return (
    <>
      <div className="fixed inset-0 z-40" onClick={onClose} />
      <div className="absolute right-0 bottom-full mb-1 w-56 bg-[#282828] rounded-xl shadow-2xl z-50 py-1.5 animate-scale-in border border-white/10">
        {/* Queue actions */}
        <button className="w-full text-left px-4 py-2.5 text-sm hover:bg-white/10 transition-colors flex items-center gap-3"
          onClick={() => act(() => addNextInQueue(track), '✓ Diputar selanjutnya')}>
        ...
        <button className="w-full text-left px-4 py-2.5 text-sm hover:bg-white/10 transition-colors flex items-center gap-3"
          onClick={() => act(() => addToQueue(track), '✓ Ditambahkan ke antrian')}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="16" height="16">
        ...
        {/* Like */}
        <button className="w-full text-left px-4 py-2.5 text-sm hover:bg-white/10 transition-colors flex items-center gap-3"
          onClick={() => act(() => toggleLike(track), liked ? 'Dihapus dari Liked Songs' : '♥ Ditambahkan ke Liked Songs')}>
          <svg viewBox="0 0 24 24" fill={liked ? '#1DB954' : 'none'} stroke={liked ? '#1DB954' : 'currentColor'} strokeWidth="2" width="16" height="16">
            <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
          </svg>
          {liked ? 'Hapus dari Liked Songs' : 'Simpan ke Liked Songs'}
        </button>
        ...
        {playlists.length > 0 && (
          <>
            <div className="h-px bg-white/10 my-1.5" />
            <p className="px-4 py-1 text-[10px] uppercase tracking-wider text-white/30 font-semibold">Tambah ke Playlist</p>
            {playlists.map(pl => (
              <button key={pl.id}
                className="w-full text-left px-4 py-2 text-sm hover:bg-white/10 transition-colors truncate flex items-center gap-3"
                onClick={() => act(() => addToPlaylist(pl.id, track), `✓ Ditambahkan ke ${pl.name}`)}>
                <div className="w-5 h-5 rounded overflow-hidden bg-[#404040] shrink-0">
    ...
    >
      ...
      <div className="flex items-center gap-1 shrink-0" onClick={e => e.stopPropagation()}>
        ...
        <div className="relative">
          ...
          {menu && <ContextMenu track={track} onClose={() => setMenu(false)} />}
        </div>
      </div>
    </div>
  )
...
export function GridCard({ track, tracks, index }: { track: Track; tracks: Track[]; index: number }) {
  const { setQueue, addToQueue, currentTrack, isPlaying, isLiked, toggleLike } = usePlayerStore()
  const [menu, setMenu] = useState(false)
  const isActive = currentTrack?.videoId === track.videoId
  const liked    = isLiked(track.videoId)
  ...
  return (
    <div className="group cursor-pointer animate-fade-in" onClick={() => setQueue(tracks, index)}>
      <div className="relative aspect-square rounded-md overflow-hidden mb-2 bg-[#282828]">
        <Image src={track.thumbnail} alt={track.title} fill
          className="object-cover transition-transform group-hover:scale-105 duration-300" unoptimized />
        <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity" />

        {/* Like btn */}
        <button
          onClick={e => { e.stopPropagation(); toggleLike(track); toast(liked ? 'Dihapus' : '♥ Liked') }}
          className={cn('absolute top-2 left-2 p-1.5 rounded-full bg-black/50 opacity-0 group-hover:opacity-100 transition-all hover:scale-110',
        ...
        <div className="absolute top-2 right-2" onClick={e => e.stopPropagation()}>
          <button onClick={() => setMenu(o => !o)}
            className="p-1.5 rounded-full bg-black/50 opacity-0 group-hover:opacity-100 transition-all hover:bg-black/70">
