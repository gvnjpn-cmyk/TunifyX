'use client'
// TunifyX v3 — LibraryView
// FIXED: playlist detail CTA for empty state
// FIXED: "Tambah lagu" button in playlist header
// FIXED: char counter on playlist name input
...
export function LibraryView() {
  const {
    playlists, createPlaylist, playContext, activeView, setActiveView,
    likes, history, clearHistory,
  } = usePlayerStore()
    ...
    <div className="px-4 md:px-8 py-6 animate-fade-in">
      <div className="flex items-center justify-between mb-5">
        ...
        {tab === 'history' && history.length > 0 && (
          <button
            onClick={() => { clearHistory(); toast('Riwayat dihapus') }}
      ...
      {/* ✅ Create playlist form with char counter */}
        ...
        <div className="mb-5 bg-[#282828] rounded-xl p-4 animate-slide-up">
          ...
          <div className="relative mb-3">
            <input
              ...
              className="w-full bg-white/10 rounded-lg px-3 py-2.5 text-sm text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-[#1DB954] pr-12"
            />
            {/* ✅ Char counter */}
            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-[#b3b3b3]/50">
              {newName.length}/50
            </span>
          </div>
      ...
      {tab === 'playlists' && (
        playlists.length === 0 ? (
          <div className="text-center py-16 animate-fade-in">
            ...
            <p className="text-sm text-[#b3b3b3]/60 mt-1">Klik "Buat" di atas untuk mulai</p>
          </div>
        ) : (
              ...
              <div key={pl.id} className="group cursor-pointer" onClick={() => setActiveView(`playlist-${pl.id}`)}>
                <div className="relative aspect-square rounded-lg overflow-hidden bg-[#282828] mb-2 flex items-center justify-center">
                  {pl.tracks[0] ? (
                    ...
                    <svg viewBox="0 0 24 24" fill="currentColor" width="40" height="40" className="text-[#b3b3b3]/30">
                  ...
                  <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-end p-3">
                    <button
                      onClick={e => { e.stopPropagation(); if (pl.tracks.length) playContext(pl.tracks, 0) }}
...
function PlaylistDetail({ playlist, onBack }: { playlist: Playlist; onBack: () => void }) {
  const { playContext, deletePlaylist, removeFromPlaylist, setActiveView } = usePlayerStore()

  return (
    <div className="px-4 md:px-8 py-6 animate-fade-in">
      <button
        onClick={onBack}
        className="flex items-center gap-2 text-[#b3b3b3] hover:text-white text-sm mb-6 transition-colors"
      >
      ...
      <div className="flex items-end gap-6 mb-8">
        <div className="w-36 h-36 md:w-48 md:h-48 rounded-xl bg-[#282828] shrink-0 overflow-hidden flex items-center justify-center shadow-2xl">
          {playlist.tracks[0] ? (
            ...
            <svg viewBox="0 0 24 24" fill="currentColor" width="56" height="56" className="text-[#b3b3b3]/20">
              <path d="M12 3v10.55A4 4 0 1 0 14 17V7h4V3h-6z"/>
            </svg>
          )}
        </div>
        <div>
          ...
          <div className="flex items-center gap-3 mt-5">
            ...
            {/* ✅ FIXED: "Tambah lagu" CTA */}
            <button
              onClick={() => setActiveView('search')}
              className="flex items-center gap-2 text-sm text-[#b3b3b3] hover:text-white transition-colors border border-white/20 hover:border-white/40 px-4 py-2 rounded-full"
            >
            ...
            <button
              onClick={() => {
                ...
                deletePlaylist(playlist.id); onBack(); toast('Playlist dihapus')
              }}
      ...
      {playlist.tracks.length === 0 ? (
        // ✅ FIXED: Empty playlist CTA
        <div className="text-center py-12 animate-fade-in">
          <div className="text-4xl mb-4">🎵</div>
          <p className="text-[#b3b3b3] font-medium mb-2">Playlist ini masih kosong</p>
          <p className="text-[#b3b3b3]/60 text-sm mb-6">Cari lagu dan klik ⋮ → tambah ke playlist ini</p>
          <button
            onClick={() => setActiveView('search')}
            className="flex items-center gap-2 mx-auto text-sm bg-white text-black font-semibold px-6 py-3 rounded-full hover:scale-105 transition-transform"
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" width="16" height="16">
